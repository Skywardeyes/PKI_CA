...
- **可选组件**：FastAPI Web 控制台（调用 PowerShell 脚本）、公开 `/repo/` 分发链与 CRL、`audit.jsonl` 模拟管理员侧留痕。

## 4. 实施过程

- 环境准备：OpenSSL、PowerShell；可选 Python 与 `web/` 虚拟环境（见 `web/README.md`）。
- 环境准备：OpenSSL、Python 与 `web/` 虚拟环境（见 `web/README.md`）；PowerShell 仅作为 Web 后端脚本引擎依赖。
- 现场从零：`docs/02-执行手册.md` **第 4 节** 环境清零（Web「0. 演示重置」）→ **第 7 节** Web 按钮完成初始化与建 CA、签发、校验。
- Root/Intermediate 构建与链路校验；服务端/客户端证书签发与浏览器导入 P12。
- Web 控制台已支持按钮触发全部核心脚本，适合单人单机演示（减少命令行输入）。
